...
import com.javen.baiduapi.BDSpecchApiUtil;
import com.javen.face.FaceService;
import com.javen.mqtt.MqttUtil;
...
public class WeixinMsgController extends MsgControllerAdapter {
	...
	protected void processInTextMsg(InTextMsg inTextMsg){
		...
		if ("help".equalsIgnoreCase(msgContent) || "帮助".equals(msgContent)) {
			...
			.setTemplate_id("RB87iNODgY8wne0V5x-w1lFuHPvP6lNHnjj3PfoTH1Q")
			.setUrl("http://www.baidu.com/")
			.add("name", "幸运转转转", "#743A3A")
			.add("time", "2011-1-1", "#0000FF")
			.build();
...
//		render(outCustomMsg);
		//县获取mediaid  文件格式都有一个临时的id
		String mdid = inVoiceMsg.getMediaId();
		System.out.println("mdid:"+mdid);
		MediaFile md = MediaApi.getMedia(mdid);
		System.out.println(md.getFileName());
		System.out.println(md.getContentLength());
		System.out.println(md.getContentType());
		BufferedInputStream fs = md.getFileStream();
		...
		try {
			...
			while(rx!=-1){
				//写入文件
				 f = new File("C:\\Users\\wc\\git\\weixin_guide\\src\\main\\java\\test\\"+md.getFileName());
		...
		AipSpeech client =BDSpecchApiUtil.getInstance();
        // 调用接口
        JSONObject res = client.asr(f.getAbsolutePath(), "amr", 8000, null);
	        System.out.println(res.toString(2));
        JSONArray rs = (JSONArray) res.get("result");
        System.out.println(rs.get(0));
		
		String str = rs.get(0)+"";
        OutTextMsg ot = new OutTextMsg(inVoiceMsg);
       
      
        //人工智能策略
        //匹配字符
        if(str.contains("天气")){
        	 ot.setContent("今天天气晴 25摄氏度");
        }else if(str.matches(".*开.*灯.*")){
        	String top="";
        	if (str.contains("客厅")) {
        		top="kt";
			}
        	...
        	ot.setContent("已经为您开灯");
        }else if(str.matches(".*关.*灯.*")){
        	String top="";
        	if (str.contains("客厅")) {
        		top="kt";
			}
        	...
        	ot.setContent("已经关灯");
        }else if(str.contains("星座")){
        	ot.setContent("您的星座是 大熊星座");
        }else {
        	ot.setContent("抱歉我听不懂");
		}
        render(ot);
